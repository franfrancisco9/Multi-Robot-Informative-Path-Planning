...
class InformativePathPlanning:
    def __init__(self, scenario, lambda_param=1.0, beta_t=2.0, n_waypoints=200, d_waypoint_distance=2.5):
    ...
    def generate_path(self):
        ...
        measurements = self.scenario.simulate_measurements(np.array([current_position]))
        self.scenario.gp.fit(np.array([current_position]), np.log10(measurements))  # Assuming log10 scale for compatibility
    ...
    def run(self):
        self.generate_path()
        measurements = self.scenario.simulate_measurements(np.array(self.obs_wp))
        self.scenario.gp.fit(np.array(self.obs_wp), np.log10(measurements))  # Assuming log10 scale for compatibility
        Z_pred, std = self.scenario.predict_spatial_field(np.array(self.obs_wp), measurements)
